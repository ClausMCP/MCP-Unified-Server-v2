...
class BaseMCPServer:
    ...
    def handle_tool_call(self, req_id, params):
        ...
        try:
            ...
            # Обработка None: если инструмент не вернул результат (например, ушёл в фон)
            if result is None:
...
class ConversationMemory:
    ...
    def __init__(self, db_path: str = MEMORY_DB_PATH, max_entries: int = MAX_MEMORY_ENTRIES):
        self.db_path = db_path
        self.max_entries = max_entries
        self._lock = threading.RLock()  # RLock to prevent deadlock
    ...
    def add(self, op: str, paths: Union[str, dict], status: str,
        ...
        # Автоматическая архивация при переполнении диалога вынесена за пределы блокировки для устранения deadlock
